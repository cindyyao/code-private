...
datarun{1} = load_data('/Analysis/xyao/2013-04-24-0/data003-map/data003-map', opt);
% datarun{1} = load_params(datarun{1}, 'cell_type_depth', 1);

opt = struct('verbose',1,'load_params',1,'load_neurons',1, 'load_ei',1);
datarun{2} = load_data('/Analysis/xyao/2013-04-24-0/data001-map/data001-map', opt);
...
datarun{2}.names.stimulus_path = '/Volumes/lab/Analysis/2013-04-24-0/stimuli/s01';
datarun{2} = load_stim(datarun{2}, 'user_defined_trigger_set', [1:2:repeat*light_level*2], 'find_trigger', 0);


cell_type = {'on transient', 'off brisk transient1', 'off brisk transient2',... 
    'off brisk transient3','off transient1', 'off transient2', 'off sustained', 'DS'};
...
elseif cell_type_n == 4
    semilogx(params, sens, 'g');
elseif cell_type_n == 5
    semilogx(params, sens, 'y');
elseif cell_type_n == 6
    semilogx(params, sens, 'c');
else
    semilogx(params, sens, 'k');
