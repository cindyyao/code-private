...
for i = 1:n
    ...
    id = [cell_id0' cell_id2' cell_id6'];
    idx = [cell_idx0' cell_idx2' cell_idx6'];
    cell_id026{i} = id;
    cell_idx026{i} = idx;
    cell_id_matched026 = [cell_id_matched026; id];
    cell_idx_matched026 = [cell_idx_matched026; idx];
...
% get parameters
datarun2 = compute_sta_fits_sequence(datarun2, cell_id_matched026(:, 1), 'verbose', true);
datarun6 = compute_sta_fits_sequence(datarun6, cell_id_matched026(:, 2), 'verbose', true);
...
for i = 1:n
    ...
    for j = 1:cell_numb
        ...
        if isempty(params2) == 1 || isempty(params6) == 1;
        ...
        params_temp(j, 1, 2) = params6.center_point_x;
        params_temp(j, 2, 2) = params6.center_point_y;
    ...
    params26{i} = params_temp;
end

%% plot mosaic

cell_type_n = 5;
params_temp = params26{cell_type_n};
...
for k = 1:n;
    ...
    if isempty(index) == 0
       for i = 1:cell_numb
           ...
           sta = datarun6.stas.stas{index(i, 3)};
           if k == 1 || k == 2
              [~, b] = max(sta(:));
           else
              [~, b] = min(sta(:));
    ...
    sta_f_weighted{k, 3} = sta_f6;
    
    sta_f_n(k, 1) = sta_f_n0;
    sta_f_n(k, 2) = sta_f_n2;
    sta_f_n(k, 3) = sta_f_n6;
end
...
save('RFS130214_map3.mat', 'fit_sta_c', 'params_c', 'sta_f_weighted',...
    'sta_c', 'n', 'd0', 'd2', 'd6', 'f0', 'f2', 'f6', 'cell_type')
