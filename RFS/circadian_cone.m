...
datarun{1} = load_data('/Analysis/xyao/2013-02-21-0/data004/data004', opt);
...
datarun{2} = load_data('/Volumes/lab/analysis/2013-07-01-0/data008/data008', opt);

cell_type = {'ON brisk transient', 'ON transient', 'ON slow transient', ...
    'OFF brisk transient', 'OFF brisk transient2', 'OFF brisk transient large', ...
    'OFF transient', 'OFF transient2', 'OFF sustained'};
n = length(cell_type);
...
save('circadian_cone.mat', 'datarun')

%% cell types summaries

% get cell ids
cell_type = {'ON brisk transient', 'ON transient', 'OFF brisk transient', 'OFF transient', 'OFF sustained'};
...
save('circadian_cone.mat', 'sta_c', '-append')
...
for ct = 1:n
    for i = 1:2
        ...
        temp_tf{i} = zeros(frame_n(i), cell_num);
        ns = [];
...
save('circadian_cone.mat', 'Spatial_Profile', 'Temporal_Filter', '-append')
...
for ct = 1:n
    ...
    DOT_temp{cd} = zeros(cell_num, 1);    
        for cn = 1:cell_num
            tf_smooth = interp1(-frame_n(cd)+1:0, temporal_filter{ct}{cd}(:, cn), -frame_n(cd)+1:smooth_f:0);
...
% surround strengths
    ...
    legend('Day','Night', 'location', 'northeast');
    hold on;
