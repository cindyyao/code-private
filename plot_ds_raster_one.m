function [] = plot_ds_raster_one(DS, raster, cell_idx, cell_id, Title, x, y, print_out)
...
%     set(FigHandle, 'Position', [1 1 1080 1080])
    for j = 1:length(DS)
        if ~isempty(raster{j}{cell_idx}) && time <= length(DS{j}.rho)
            ...
            set(P, 'Visible', 'off')
            hold on
            compass(DS{j}.U{end-time+1}(cell_idx), DS{j}.V{end-time+1}(cell_idx), color(j));
            polar(tt, DS{j}.rho{end-time+1}(cell_idx, :), color(j));
            polar_theta_off(h)
            for i = 2:9
                subplot(xx, yy, idx(i)); 
                plot_raster(squeeze(raster{j}{cell_idx}(1, end-time+1, i-1, :)), 0, 8, 'color', color(j))
%                 if i == 4
%                     title(Title{j})
%                 end 
                if mod(idx(i), yy) == 1
