function [profiles, h] = interdig_compute_and_plot_neighbor_rf(datarun,cell_types, varargin)
...
%plot([1:profile_points]/profile_points*(2*extension_factor+1),[avg_profile{1}],'Color',color);
    h(tt) = plot([1:profile_points]/profile_points*(2*extension_factor+1),[avg_profile{1} - mean(avg_profile{1}(1:10))],'Color',color);
    % change tick marks
    %set(gca,'TickDir','out');
