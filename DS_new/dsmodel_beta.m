    ...
    for width = 1:length(Alpha2)
        ...
        for k = 1:length(beta)
            for ctr = 1:length(contrast)
                Precision_ratio(width, k, ctr) = run_poiss_Detection_beta(c50,alpha1,phi,alpha2,r_base,ymax,contrast(ctr),repeat_n,beta(k));
                [error(width,k,ctr), theta_e{width,k,ctr}] = run_poiss_MLdecoder_beta(c50,alpha1,phi,alpha2,r_base,ymax,contrast(ctr),repeat_n,beta(k));
        %         [width ctr]
...
figure
for beta = 1:6
    plot(theta, Precision_rod{1}(:,beta,5));
    hold on
end
