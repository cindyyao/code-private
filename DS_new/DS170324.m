...
datadg = load_data('/Volumes/lab/analysis/2017-03-24-0/data009-sorted/data009-sorted', opt);
datadg.names.stimulus_path = '/Volumes/lab/analysis/2017-03-24-0/stimuli/s09.txt';
...
datarun = load_data('/Volumes/lab/analysis/2017-03-24-0/data001-010-map/data001-010-map', opt);
time_points = [400 1350 1947 2347 3247 5099 5499 6449];
datamb_temp(1:9) = split_datarun(datarun, time_points);
datamb{1} = datamb_temp{2};
datamb{1}.names.stimulus_path = '/Volumes/lab/analysis/2017-03-24-0/stimuli/s02.txt';
datamb{1} = load_stim(datamb{1}, 'user_defined_trigger_set', [1:2:560]);
datamb{1}.stimulus.triggers = datamb{1}.stimulus.triggers';

datamb{2} = datamb_temp{5};
datamb{2}.names.stimulus_path = '/Volumes/lab/analysis/2017-03-24-0/stimuli/s05.txt';
datamb{2} = load_stim(datamb{2}, 'user_defined_trigger_set', [1:2:560]);
datamb{2}.stimulus.triggers = datamb{2}.stimulus.triggers';

datamb{3} = datamb_temp{8};
datamb{3}.names.stimulus_path = '/Volumes/lab/analysis/2017-03-24-0/stimuli/s08.txt';
...
for drug = 1:3
    for dir = 1:4
        for cc = 1:length(id_dir_mb{dir})
            ...
            if drug == 2
                bgnd_firing{dir}(drug, cc) = length(spikes_temp(spikes_temp < 10))/10;
            else
...
for dir = 1:4
    subplot(2,2,dir)
    for drug = 1:3
        errorbar(ctr_x, mean(exciseRows_empty(response_pmax_norm{drug}{dir})), std(exciseRows_empty(response_pmax_norm{drug}{dir}))/sqrt(size(exciseRows_empty(response_pmax_norm{drug}{dir}), 1)), 'color', color(drug));
        hold on
    end
