...
datarun = load_data('/Volumes/lab/analysis/2016-02-18-0/data000-2-3-map/data000-2-3-map', opt);
time_points = [720 1440];
...
datarun = load_data('/Volumes/lab/analysis/2016-02-18-0/data005-7-map/data005-7-map', opt);
time_points = [720 1440];
datamb(4:6) = split_datarun(datarun, time_points);
...
for i = 1:n    
    [NumSpikesCell, MaxRate, StimComb] = get_spikescellstim_mb(datamb{i},ds_id,duration,bin_size);
...
%                     y_temp = MB_NDF{drug, d}.RHO{T,BW,cl}(idx_dir{i}(cc), :);
                    y_temp = MB_NDF{drug, d}.RHO{T,BW,cl}(idx_dir{i}(cc), :)/bin_size;
...
%% prefer direction spike # change
for d = 1:2
    for drug = 1:3
        for ct = 1:4
            for cl = 1:3
                prefer_spike_number{drug, d}{ct}{cl} = max(rho_mb{drug, d}{ct}{cl}, [], 2);
            end
        end
    end
end
...
for d = 1:2
    for cl = 1:3
        h = figure;
        set(h, 'Position', [1,1,750,700])
        for ct = 1:4
            subplot(2, 2, ct)
            y = [];
            for drug = 1:3
                y = [y prefer_spike_number{drug, d}{ct}{cl}];
            end
            plot(y', 'ko-')
            xlim([0 4])
            xtick = {'', 'control', 'drug', 'wash', ''};
            set(gca,'XTicklabel',xtick)
            ylabel('max rate of PD(Hz)')
%             ylabel('spike # of PD(Hz)')
            title(cell_type{ct})
        end
    end
end
...
for d = 1:2
    for cl = 1:3
        ...
        for ct = 1:4
            ...
            plot(y(:, 1), y(:, 2), 'o')
%             xlabel('control max rate of PD(Hz)')
            xlabel('control spike # of PD')
%             ylabel('drug max rate of PD(Hz)')
            ylabel('drug spike # of PD')
            title(cell_type{ct})
            xlim([0 max(y(:, 1))*1.1+1])
            ylim([0 max(y(:, 2))*1.1+1])
        end
    end
end