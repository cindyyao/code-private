...
ylabel('log(R*/rod)')
title('Pc = 0.83')
xtick = {'superior'; 'anterior'; 'inferior'};
...
n_idx = ceil(mod(p_idx{1}+4-0.1,8));
for i = [1]
    clear max_rate_prefer_temp max_rate_null_temp
    for tp = 1:length(DG{1}.RHO)
        for cc = 1:length(ds_id)
            max_rate_prefer_temp{cc}(tp) = DG{i}.RHO{tp}(cc, p_idx{1}(cc));
...
% for ct = 1:4
    ...
    errorbar(log10(Irel(1:a)), Pc_temp(1:a), Pc_ste_temp(1:a), 'bo');
    hold on
    h = plot(x-4, y+0.5, 'color', 'b');
% end
xlabel('R*/rod')
ylabel('Pc')
...
% xlim([-inf 0.5])
%%
cell_type = {'ON transient', 'ON sustained', 'OFF transient'};
ct = 3;
id_all = id_nds_flash{ct};

figure
for i = 1:length(id_all)
ts = 2;
id = id_all(i);
idx = get_cell_indices(dataflash, id);
raster_all{ct} = get_raster(dataflash.spikes{idx}, ...
            dataflash.triggers(trigger_set_i{ts}(1)), 'start', 0, 'stop', ...
            dataflash.DfParams.interFlashInt*10, 'plot', 0);
subplot(length(id_all), 11,[(i-1)*11+1:(i-1)*11+10])
X = [raster_all{ct}{1}'; raster_all{ct}{1}'];
Y = [ones(size(raster_all{ct}{1}'))*0.1; ones(size(raster_all{ct}{1}'))*0.9];
line(X, Y, 'color', 'b');
hold on
Xt = dataflash.triggers(trigger_set_i{ts}(1:10));
Xt = Xt - Xt(1);
X = [Xt'; Xt'];
Y = [zeros(size(Xt')); ones(size(Xt'))];
line(X, Y, 'color', 'r');
set(gca, 'YTick', [])
...
raster_all_{ct} = sort(cell2mat(get_raster(dataflash.spikes{idx}, ...
            dataflash.triggers(trigger_set_i{ts}), 'start', 0, 'stop', ...
            dataflash.DfParams.interFlashInt, 'plot', 0)));
        
subplot(length(id_all), 11,(i-1)*11+11)
hist(raster_all_{ct}, XX)
xlim([0 3])
end
...
DG_nds = sort_direction(dscellanalysis(NumSpikesCell, StimComb,datadg));
raster_dg_nds = get_ds_raster(datadg, nonds_id);
