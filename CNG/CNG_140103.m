...
for i = 1:3
    ...
    start_I{i} = [1; find(trigger_diff{i} >6.1)+1]; % after turning on the output, the duration of each trial is 3 sec or 6 sec.
    start_time{i} = datarun{i}.triggers(start_I{i}); % convert the index of triggers to time
end
start_time{1}(12) = []; % delete the wrong triggers
start_I{1}(12) = [];
...
figure
errorbar(unique(stimuli{1}), mean(sensitivity_all{1}), std(sensitivity_all{1})/sqrt(size(sensitivity_all{1}, 1)), 'b')
hold on 
errorbar(unique(stimuli{2}), mean(sensitivity_all{2}), std(sensitivity_all{2})/sqrt(size(sensitivity_all{2}, 1)), 'r')
errorbar(unique(stimuli{3})*0.0144, mean(sensitivity_all{3}), std(sensitivity_all{3})/sqrt(size(sensitivity_all{3}, 1)), 'k')
...
%%
     ...
     X = [datarun{3}.triggers'; datarun{3}.triggers'];
     Y = [ones(1, length(datarun{3}.triggers))*0.1; ones(1, length(datarun{3}.triggers))];
     line(X, Y, 'color', 'b');
