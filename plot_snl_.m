...
% plot mean
[X,Y] = curve_from_binning(gen_signal,spikes,'average_y','mean','average_x','mean','num_bins',20);
...
if ~isempty(params.fit)
    ...
    switch params.fit.type
        case 'exp'
            ...
            fit_name = sprintf('(%0.2f,%0.2f)',params.fit.b,params.fit.a);
        otherwise
            error('fit type ''%s'' not recognized',params.fit.type)
    end
    % plot it
    plot(X,fit_fcn(X),'r-','Parent',plot_axes)
    % append to title
